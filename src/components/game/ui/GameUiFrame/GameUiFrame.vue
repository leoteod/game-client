<template>
  <div v-if="show" class="frame__bg" />
  <div data-tauri-drag-region class="frame" :class="{ 'frame--off': !show }">
    <div>
      <h1 class="pl-4">
        <span class="text-xs uppercase font-medium">Game Client</span>
        <sup class="text-xxs ml-2 text-red-500 font-medium">v1.0.0-beta</sup>
      </h1>
    </div>
    <div v-if="show">
...
onMounted(() => {
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore
  const isTauri = !!window.__TAURI_INVOKE__;
  if (isTauri) {
    show.value = true;
    nextTick(() => {
      document
...
.frame {
  ...
  &--off {
    @apply bg-black text-white bg-opacity-90 justify-center;
  }
  &__bg {
